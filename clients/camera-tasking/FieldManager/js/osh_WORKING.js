...
/**
 * @classdesc This datasource provides parsing to Arduino Weather Station.
 * @class OSH.DataReceiver.DataSourceArduinoTemperature
 * @augments OSH.DataReceiver.DataSource
 */
OSH.DataReceiver.DataSourceArduinoTemperature = OSH.DataReceiver.DataSource.extend({
   ...
   * @param {string} data the data to parse
   * @returns {number} the extracted timestamp
   * @memberof OSH.DataReceiver.DataSourceArduinoTemperature
   * @instance
   */
  parseTimeStamp: function(data){
    var rec = String.fromCharCode.apply(null, new Uint8Array(data));
    var tokens = rec.trim().split(",");
    return new Date(Number(tokens[0])*1000).getTime();
  },
...
/**
 * @classdesc This datasource provides parsing to Arduino Weather Station.
 * @class OSH.DataReceiver.DataSourceArduinoHumidity
 * @augments OSH.DataReceiver.DataSource
 */
OSH.DataReceiver.DataSourceArduinoHumidity = OSH.DataReceiver.DataSource.extend({
   ...
   * @param {string} data the data to parse
   * @returns {number} the extracted timestamp
   * @memberof OSH.DataReceiver.DataSourceArduinoHumidity
   * @instance
   */
  parseTimeStamp: function(data){
    var rec = String.fromCharCode.apply(null, new Uint8Array(data));
  ...
  parseData: function(data){
    var rec = String.fromCharCode.apply(null, new Uint8Array(data));
    var tokens = rec.trim().split(",");
    var humidity = parseFloat(tokens[1]);
...
OSH.DataSender.PtzTasking = OSH.DataSender.DataSink.extend({
    ...
    getCommandData:function(values) {
        ...
        if(values.tilt != null) {
          cmdData += "tilt," + values.tilt + " ";
        }

        if(values.pan != null) {
          cmdData += "pan," + values.pan + " ";
        }

        if(values.zoom != null) {
          cmdData += "zoom," + values.zoom + " ";
        }

        /*Add support for ptzPos*/
        if(values.ptzPos != null) {
          cmdData += "ptzPos," + values.ptzPos.pan + "," + values.ptzPos.tilt + "," + values.ptzPos.zoom + " ";
        }
...
OSH.UI.Nvd3CurveChartView = OSH.UI.View.extend({
    ...
    var maxY = 100;
		...
		if (typeof (options) != "undefined") {
			...
			if (options.showLegend !== undefined && options.showLegend !== null) {
				showLegend = options.showLegend;
			}

			if (options.showXAxis !== undefined && options.showXAxis !== null) {
				showXAxis = options.showXAxis;
			}

			if (options.showYAxis !== undefined && options.showYAxis !== null) {
				showYAxis = options.showYAxis;
			}

			if (options.useInteractiveGuideline !== undefined && options.useInteractiveGuideline !== null) {
      ...
      if (options.axisControl !== undefined && options.axisControl !== null) {
        if (options.axisControl.constrainAxes) {
          if (options.axisControl.minY) {
            this.minY = options.axisControl.minY;
          }
          if (options.axisControl.maxY) {
            this.maxY = options.axisControl.maxY;
          }
        }
      }
    }
...
OSH.UI.CesiumView = OSH.UI.View.extend({
    ...
    if (options.flyToLoc)
    {
      ...
      /////////////// Displays Lat/Lon on mouseover ///////////////////////////
      ///////// Taken from example given on Cesium website ////////////////////
      // https://cesiumjs.org/Cesium/Apps/Sandcastle/index.html?src=Picking.html&label=undefined
      var handler;
      ...
      handler.setInputAction(function(dbClick) {
        // var cartesian = viewer.camera.pickEllipsoid(dbClick.endPosition, scene.globe.ellipsoid);
        var cartesian = viewer.camera.pickEllipsoid(dbClick.position, scene.globe.ellipsoid);
        var ray = viewer.camera.getPickRay(dbClick.position);
        ...
        if (cartesian)
        {
          ...
          // };
          //
          // $.post("/Clients/VideoDisplay/dump.html", JSON.stringify(cursorLoc), function(response) {
	...
	addMarker : function(properties) {
		...
		else
		{
          ...
          rotation : Cesium.Math.toRadians(properties.orientation.heading),
    ...
    //-------------------------------------- Model Scaling ------------------------------------------------------------------------------//
    // Calculate distance from Camera to Surface used for icon scaling --> scales relative to camera distance from surface of earth
    var cameraPosition = this.viewer.scene.camera.positionWC;
    var ellipsoidPosition = this.viewer.scene.globe.ellipsoid.scaleToGeodeticSurface(cameraPosition);
    var distance3d = Cesium.Cartesian3.magnitude(Cesium.Cartesian3.subtract(cameraPosition, ellipsoidPosition, new Cesium.Cartesian3()));
    var distance2d = (this.viewer.scene.camera.frustum.right - this.viewer.scene.camera.frustum.left) * 0.5;
    //-----------------------------------------------------------------------------------------------------------------------------------//
    ...
    //---------------------- Model Scaling ---------------------------//
    var modelScale = 4; // initial icon (model) scale
    //---------------------------------------------------------------//
    ...
    if (this.viewer.scene.mode == 2) {
      modelScale = distance2d*(99/4800) + 1;
    }
...
OSH.UI.PtzTaskingView = OSH.UI.View.extend({
    initialize: function (divId, options) {
        ...
        if(typeof (options) !== "undefined") {
            ...
            if (options.taskInterval) {
                this.taskInterval = options.taskInterval;
            }

            // if (options.headingOffset !== null) {
            //   this.headingOffset = options.headingOffset;
            // }
        }
        ...
        document.querySelector('#'+this.rootTag.id+ " >  .ptz > .moveTopLeft").onclick = function(){this.onTiltPanClick(-1*ptIncrement,ptIncrement)}.bind(this);
        document.querySelector('#'+this.rootTag.id+ " >  .ptz > .moveTopRight").onclick =  function(){this.onTiltPanClick(ptIncrement,ptIncrement)}.bind(this);
        document.querySelector('#'+this.rootTag.id+ " >  .ptz > .moveRight").onclick =  function(){this.onPanClick(ptIncrement)}.bind(this);
        document.querySelector('#'+this.rootTag.id+ " >  .ptz > .moveLeft").onclick =  function(){this.onPanClick(-1*ptIncrement)}.bind(this);
        document.querySelector('#'+this.rootTag.id+ " >  .ptz > .moveDown").onclick =  function(){this.onTiltClick(-1*ptIncrement)}.bind(this);
        document.querySelector('#'+this.rootTag.id+ " >  .ptz > .moveBottomLeft").onclick = function(){this.onTiltPanClick(-1*ptIncrement,-1*ptIncrement)}.bind(this);
        document.querySelector('#'+this.rootTag.id+ " >  .ptz > .moveBottomRight").onclick =  function(){this.onTiltPanClick(ptIncrement,-1*ptIncrement)}.bind(this);
    ...
    addSources:function(taskersArr) {
        ...
        for(var i in taskersArr) {
            ...
            option.value = taskersArr[i].id;
            selectTag.add(option);

            this.taskSrcIds.push(taskersArr[i].id); // Add each source ID to this array
            this.registerSource(taskersArr[i].id); // Register a listener for each task source
        }
    },

    /**
    * PTZ task source listener
    **/
    registerSource: function(srcId) {
      OSH.EventManager.observe(OSH.EventManager.EVENT.DATA+"-"+srcId, function (event) {
        for (var k in this.taskSrcIds) {
          if (srcId == this.taskSrcIds[k]) {
            // create tuple [lat,lon,alt] for each tasking data source
            this.taskSrcData[k] = event.data.data.lat + "," + event.data.data.lon + "," + event.data.data.alt;
          }
        }
      }.bind(this));
    },
    ...
    onSelectedSources : function(event) {
        ...
        for (var q in this.taskSrcIds) {
          // Get index (q) of currently selected source id in taskSrcIds[]
          if (sourceId == this.taskSrcIds[q]) {
            this.selectedSource = true;
            this.srcIndex = q; // Grab and store the correct index as a class variable
            break;
          }
        }

        // Set interval to ultimately send ptz tasking command every N milliseconds
        this.intervalID = setInterval(
          (function(self) {
            return function() {
              self.sendTaskSourceCommand(self.taskSrcData); // Send current tasking source data to send command function
            }
        })(this), this.taskInterval); //this.taskInterval can be set in osh-config

         // Add intervalID corresponding to most recently selected tasker choice to intervalIdArr
         // This array will later be checked for size and retain only the last element
        this.intervalIdArr.push(this.intervalID);
    },

    sendTaskSourceCommand: function(taskData) {
      ...
      var str, lat, lon, alt, posVal;

      // parse current tuple [lat,lon,alt]
      str = this.taskSrcData[this.srcIndex].split(",");
      lat = str[0];
      lon = str[1];
      alt = str[2];

      // Will eventually just send this to data sender and bearing calculations below will be removed
      posVal = [lat, lon, alt];

      // For now, calculate bearing between camera and target,
      // then pan to that angle
      var deltaLat, deltaLon, bearing;
      deltaLat = Math.abs(this.camLat - lat);
      deltaLon = Math.abs(this.camLon - lon);
      bearing = Math.atan2(deltaLon,deltaLat);
      bearing = bearing*180.0/Math.PI;

      // 4th quadrant
      if (lat <= this.camLat && lon >= this.camLon) {
      ...
      // bearing = bearing - this.headingOffset;
