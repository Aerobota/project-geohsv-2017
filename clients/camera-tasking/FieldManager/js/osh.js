    ...
    if (options.flyToLoc)
    {
      ...
      handler.setInputAction(function(dbClick) {
        ...
        if (cartesian)
        {
          ...
          var longitudeString = Cesium.Math.toDegrees(cartographic.longitude).toFixed(14);
          var latitudeString = Cesium.Math.toDegrees(cartographic.latitude).toFixed(14);

          // Get tile level-of-detail (number)
          // This will be used when calculating altitude
          var tileLevel = 11;
          ...
          // Cesium function to calculate altitude (sampleTerrain) is asynchronous,
          // so we have to put click-to-task logic inside a callback function
          // getElevation () is defined in locationTasker.js
          getElevation(latitudeString, longitudeString, tileLevel, function(alt) {
            /////////////////// Need to convert data string to array buffer ////////////////////
            //////////////// so we can simply insert using existing websocket //////////////////
            if (!alt) {
            ...
            var tempString = btoa(unescape(encodeURIComponent(dataString))); // Base 64 string
            var charList = dataString.split(''); // Split string into individual chars
            var uintArray = []; // Declare empty array

            for (var i = 0; i < charList.length; i++) {
              uintArray.push(charList[i].charCodeAt(0)); // Load uintArray with chars from string
            }

            ////////////////////// Send location array buffer to websocket //////////////////////////////
            // console.dir(new Uint8Array(uintArray).buffer); // This is what needs to be sent to websocket!
            insertClickToTaskData(new Uint8Array(uintArray).buffer);
            /////////////////////////////////////////////////////////////////////////////////////////////
